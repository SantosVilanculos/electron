...
export const useSettingsStore = defineStore('config', () => {
  ...
  watch(theme_source, async _ => await settings.set('theme_source', _));
  ...
  settings.get('theme_source').then(_ => (theme_source.value = _));
  ...
  settings.on_did_change('theme_source', _ => (theme_source.value = _));
